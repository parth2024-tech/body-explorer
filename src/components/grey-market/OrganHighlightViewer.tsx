...
export function OrganHighlightViewer({ target_organ_id }: { target_organ_id?: string }) {
  const selected    = target_organ_id || null;
  const activeLayer = "explore";
  ...
  const handleClick = useCallback((id: string, e: React.MouseEvent<SVGGElement>) => {
    // No-op for this viewer, selection is controlled externally
  }, [shouldReduceMotion, layerColor]);

  // Organ class builder
  const cls = (id: string) => {
    const base = "organ-path";
    if (!selected) return base; // reset state
    const isActive = selected === id;
    return `${base} ${isActive ? "organ-grey-market-active" : "organ-grey-market-inactive"}`;
  };
  ...
  return (
    ...
    >
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes pulse-red-emissive {
          0% { filter: drop-shadow(0 0 10px rgba(255,59,48,0.6)); opacity: 0.8; stroke-width: 1px; }
          50% { filter: drop-shadow(0 0 25px rgba(255,59,48,1)); opacity: 1; stroke-width: 2px; }
          100% { filter: drop-shadow(0 0 10px rgba(255,59,48,0.6)); opacity: 0.8; stroke-width: 1px; }
        }
        .organ-grey-market-active {
          animation: pulse-red-emissive 1.2s ease-in-out infinite;
          fill: rgba(255,59,48,0.3) !important;
          stroke: #FF3B30 !important;
          z-index: 10;
        }
        .organ-grey-market-inactive {
      ...
      <div
        ...
        style={{ background: `radial-gradient(ellipse at center, ${selected ? '#FF3B30' : layerColor}55 0%, transparent 65%)` }}
      />

      {/* ── Layer toggle (Removed for Grey Market) ── */}
      {/* Layer toggle removed */}
