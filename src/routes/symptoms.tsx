  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-8 page-enter">
      ...
      <div className="mb-8 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-4 text-center border-l-4 border-l-rose-500">
        ...
        <span className="text-xs text-[#8B8FA3]">{t("notADiagnosis")} Seek immediate professional help if you are experiencing severe pain or life-threatening symptoms.</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-10">
          {/* Symptom Explorer */}
          <div className="rounded-2xl border border-border bg-[#141826]/60 p-6">
            <h2 className="text-2xl font-bold text-[#E8E0D5] mb-4">Symptom Explorer</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-[#00E5C4] mb-2">
                    Innocent / Common Causes
                  </h3>
                  <ul className="list-disc list-inside space-y-1 text-sm text-[#8B8FA3]">
              ...
              <div className="rounded-xl bg-rose-500/5 border border-rose-500/10 p-4">
                ...
                <p className="text-[11px] text-[#8B8FA3] mb-4">
                ...
                <div className="space-y-3">
                  {selectedExplorerSymptom.redFlags.map((flag, idx) => (
                    <label key={idx} className="flex items-start gap-2.5 cursor-pointer text-xs text-[#8B8FA3]">
                      <input
                        type="checkbox"
                        checked={!!redFlagAnswers[idx]}
                        onChange={(e) => setRedFlagAnswers((prev) => ({ ...prev, [idx]: e.target.checked }))}
                        className="mt-0.5 rounded border-[#1E2844] bg-[#0A0E1A] text-rose-500 focus:ring-rose-500"
                      />
          ...
          <div>
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h2 className="text-2xl font-bold text-[#E8E0D5]">A–Z Disease Directory</h2>
            ...
            <input
              ...
              className="w-full mb-6 rounded-xl border border-border bg-[#141826]/80 px-4.5 py-3 text-sm text-[#E8E0D5] placeholder-[#8B8FA3] outline-none focus:border-[#00E5C4]/40"
            />

            {filteredDiseases.length === 0 ? (
              <p className="text-sm text-[#8B8FA3]">No matching diseases found.</p>
            ) : (
              <div className="space-y-4">
                {filteredDiseases.map((d) => (
                  <div key={d.id} className="rounded-xl border border-border bg-[#111525] p-5">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <h3 className="text-xl font-bold text-[#E8E0D5]">{d.name}</h3>
                        <span className="text-[10px] text-[#00E5C4] font-semibold uppercase tracking-wider block mt-0.5">
                          Affected Organ ID: {d.bodyPartId}
                        </span>
                      </div>
                    </div>
                    <p className="mt-3 text-sm text-[#8B8FA3]">{d.overview}</p>

                    {/* Symptoms lists */}
                    <div className="mt-4">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-[#8B8FA3] mb-2">
                        Common Symptoms & Frequency
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {d.symptoms.map((s, i) => (
                          <span key={i} className="inline-flex items-center gap-1.5 rounded bg-[#1A2038] px-2.5 py-1 text-xs text-[#E8E0D5] border border-border/40">
                            {s.text} 
                    ...
                    {/* Misconceptions */}
                    <div className="mt-4 border-l-2 border-amber-500/40 pl-4 py-1 text-xs text-[#8B8FA3]">
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-xl border border-border bg-[#141826] p-5">
            ...
            <h3 className="mt-3 text-lg font-bold text-[#E8E0D5]">Why do we feel pins & needles?</h3>
            <p className="mt-2.5 text-xs text-[#8B8FA3] leading-relaxed">
            ...
            <div className="mt-4 pt-3 border-t border-border/40 text-[10px] text-[#8B8FA3]">
          ...
          <div className="rounded-xl border border-border bg-[#141826] p-5">
            <h3 className="text-md font-bold text-[#E8E0D5] mb-3">Map Region Selector</h3>
              ...
              {[
                ...
                <button
                  ...
                  className={`p-3 rounded-lg border text-xs font-semibold text-center transition-all ${
                    selectedRegion === r.id
                      ? "bg-[#00E5C4]/15 border-[#00E5C4] text-[#00E5C4]"
